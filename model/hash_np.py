...
class MultiHeadHashNP(tf.keras.Model):
    def __init__(self, group=4, feat_dim=4096, middle_dim=512, code_length=32, k=10, emb_dim=256, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.group = group
        self.feat_dim = feat_dim

        self.encoder = vq_vae.MultiHeadVQVAE(group=group, feat_dim=feat_dim, k=k, emb_dim=emb_dim, top='relu')
        self.decoder = encodec.MultiDetDecoder(group=group, middle_dim=middle_dim, code_length=code_length, k=k)
